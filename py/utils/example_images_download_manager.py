...
class _DownloadProgress(dict):
    ...
    def snapshot(self) -> dict:
        ...
        snapshot.pop("failed_model_timestamps", None)
        return snapshot
...
class DownloadManager:
    ...
    async def start_download(self, options: dict):
        ...
        async with self._state_lock:
            ...
            if self._is_downloading:
                raise DownloadInProgressError(snapshot)
                ...
                if os.path.exists(progress_source):
                    try:
                        with open(progress_source, "r", encoding="utf-8") as f:
                            ...
                            self._progress["failed_model_timestamps"] = (
                                saved_progress.get("failed_model_timestamps", {})
                            )
                            logger.debug(
    ...
    async def _download_all_example_images(
        ...
        try:
            ...
            failed_count = len(self._progress["failed_models"])
            if failed_count > 0:
                logger.warning(
                    "%s models failed to download example images. These will be retried in 24 hours.",
                    failed_count,
                )
    ...
    async def _process_model(
        ...
        try:
            ...
            if model_hash in self._progress["failed_models"]:
                if existing_files:
                    logger.debug(
                        f"Skipping failed model with existing files: {model_name}"
                    )
                    return False

                failure_time = self._progress["failed_model_timestamps"].get(
                    model_hash, 0
                )
                retry_interval = 24 * 60 * 60

                if time.time() - failure_time < retry_interval:
                    logger.debug(
                        f"Skipping recently failed model: {model_name} (retry in {int((retry_interval - (time.time() - failure_time)) / 60)} minutes)"
                    )
                    return False

                logger.info(
                    "Retrying previously failed model %s (%s) - %.1f hours since last failure",
                    model_name,
                    model_hash[:8],
                    (time.time() - failure_time) / 3600,
                )
                self._progress["failed_models"].discard(model_hash)
                self._progress["failed_model_timestamps"].pop(model_hash, None)
                # FALL THROUGH to normal processing with variables already set
            ...
            if civitai_payload.get("images"):
                ...
                if failed_urls:
                    ...
                    self._progress["failed_model_timestamps"][model_hash] = time.time()
                    self._progress["processed_models"].add(model_hash)
                    logger.info(
    ...
    def _save_progress(self, output_dir):
        ...
        try:
            ...
            progress_data = {
                ...
                "failed_model_timestamps": self._progress.get(
                    "failed_model_timestamps", {}
                ),
